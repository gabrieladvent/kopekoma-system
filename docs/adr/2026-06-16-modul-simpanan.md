...
| 4a | `MemberHolidaySavingResource` — config Hari Raya per anggota/tahun; **tambah `LogsActivity` ke model `MemberHolidaySaving`** (belum ada → langgar konvensi audit wajib) — *defer-able* | S | setelah 1d | Pending |
...
- [ ] Double-submit form key sama → 1 baris, sukses idempoten; **payload beda dengan key sama → warning, bukan silent** (D4). <!-- source: code -->
