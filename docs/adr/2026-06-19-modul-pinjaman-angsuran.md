...
**Author:** Ribka Restu
**Date:** 2026-06-19
**Status:** Implemented — sisa: security/deploy review (3c batch potong gaji angsuran selesai 2026-06-26)
...
| Service batch angsuran terpisah delegasi ke `LoanPaymentService::pay()` (D6) | Reuse penuh validasi/idempotency/auto-Lunas/refund; tak ada cabang lintas-domain | Dua engine batch berdampingan (savings + angsuran) | **Chosen** (3c, 2026-06-26) |
| Generalisasi `BatchSalaryDeductionService` utk angsuran (D6) | DRY satu engine | Abstraksi prematur lintas-domain: schedule FIFO + refund pelunasan asing bagi setoran simpanan | Rejected |
...
| 3c | Pembayaran angsuran via **batch potong gaji per OPD** (`BatchInstallmentPaymentService` delegasi ke `LoanPaymentService::pay()` + `BatchInstallmentPayment` page; service TERPISAH, bukan generalisasi — D6). **13 test GREEN** | M | setelah 3a | **Done** |
...
- ✅ **(D6) Engine batch angsuran** — RESOLVED (eksekusi 3c, 2026-06-26): **service terpisah** `BatchInstallmentPaymentService`, BUKAN generalisasi `BatchSalaryDeductionService` (domain berbeda: FIFO schedule + auto-Lunas + refund pelunasan). Service hanya mengorkestrasi (lock per-OPD, log batch satu peristiwa, pra-validasi `≥ tagihan` = abort) lalu **delegasi tiap baris ke `LoanPaymentService::pay()`** → reuse penuh validasi/idempotency/auto-Lunas/refund atomik/audit-per-anggota. Menghindari abstraksi prematur lintas-domain (verdict Alternatives).
...
- **2026-06-26 v11 (EKSEKUSI 3c — batch potong gaji angsuran)**: Item terakhir yang pending selesai. **(D6 RESOLVED)** dipilih **service terpisah** `BatchInstallmentPaymentService` (bukan generalisasi engine Simpanan): mengorkestrasi lock per-OPD + pra-validasi `≥ tagihan` (abort bila kurang, anti-korupsi total-level) + log batch satu peristiwa, lalu **delegasi tiap baris ke `LoanPaymentService::pay()`** → reuse penuh FIFO/idempotency/auto-Lunas/refund SWP+Tab atomik/audit-per-anggota. UI: `BatchInstallmentPayment` Filament page (analog `BatchSalaryDeduction` Simpanan; baris per **(anggota, pinjaman aktif)**, prefill tagihan jadwal terlama belum bayar), tombol "Batch Potong Gaji" di `ListInstallments`. Permission **reuse `access_batch_salary_deduction`** (Petugas+, tanpa seed/shield baru). **Self-review keamanan (lane security-reviewer): invariant per-OPD ditegakkan server-side** — service `fail-closed` melewati `schedule_id` yang bukan milik anggota OPD terpilih (`schedule->loan->member->agency_id`), jadi payload Livewire yang diutak-atik tak bisa membuat batch satu OPD membayar angsuran OPD lain (audit `agency_id` jujur). Tidak menambah privilege (Petugas memang sudah bisa bayar angsuran mana pun via `InstallmentResource`); ini menjaga integritas framing/audit batch. **15 test baru GREEN** (service+page, termasuk OPD-scoping & skip-saat-loan-tak-Cair-tetap-commit-baris-valid), **405 test GREEN total** (MySQL `kopekoma_test`, 1465 assertion), pint clean. Sisa modul: deploy review (catatan: tanpa migrasi/permission baru → deploy = `view:clear`+`route:clear`+`filament:cache-components` bila panel di-cache; risiko rendah).
