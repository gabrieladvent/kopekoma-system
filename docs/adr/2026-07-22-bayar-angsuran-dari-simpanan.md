...
- [ ] `LoanPaymentService` jalur debit + reversal berpasangan + test hijau — jalur debit (1c) done + test hijau; **reversal berpasangan (1d) belum**
  <!-- source: code -->
...
| 1d | `reverse()`: balik debit berpasangan via `installment_id`, **exclude yang sudah dibalik**, **bypass guard `memberInactive`** (reverse debit selalu boleh) | M | setelah 1c | Pending |
| 1e | Permission `pay_installment_from_savings` (Pengurus) + seeder + `InstallmentPolicy::payFromSavings`; **`InstallmentPolicy::reverse` gate `reverse_loan` untuk `saldo_simpanan`** (cegah privilege inversion) | M | setelah 1c | Pending |
| 1f | Guard non-reversible terpisah: `SavingsWithdrawalPolicy::reverse` tolak baris ber-`installment_id` + sembunyikan aksi di ketiga UI (`SavingsWithdrawalResource`/`SavingsWithdrawals`/`SavingsWithdrawalDetail`) + **guard layer mutasi** (defense-in-depth) | M | setelah 1a | Pending |
