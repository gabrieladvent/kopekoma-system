    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <h2 class="mt-2 text-2xl font-bold tracking-tight text-text">{{ $installment->loan?->member?->full_name ?? '—' }}</h2>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <x-ui.card>
                ...
                <div class="rounded-xl px-4 py-4 {{ $installment->is_reversal ? 'bg-danger/5 ring-1 ring-inset ring-danger/15' : 'bg-success/5 ring-1 ring-inset ring-success/15' }}">
                    <p class="text-xs font-medium uppercase tracking-wide {{ $installment->is_reversal ? 'text-danger' : 'text-success' }}">
                        {{ $installment->is_reversal ? 'Nominal Dibatalkan' : 'Total Dibayar' }}
                    </p>
                    <p class="mt-1 text-3xl font-bold tabular-nums {{ $installment->is_reversal ? 'text-danger' : 'text-success' }}">
                ...
                <dl class="mt-5 grid grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-text">{{ $installment->payment_date?->translatedFormat('d M Y') }}</dd>
                    </div>
                    <div>
                        <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
                            <x-ui.icon name="clock" class="h-3.5 w-3.5" /> Jatuh Tempo
                        </dt>
                        <dd class="mt-1 text-sm text-text">{{ $installment->due_date?->translatedFormat('d M Y') ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm">
                            @if ($installment->loan)
                                <a href="{{ route('loans.show', $installment->loan) }}" wire:navigate class="font-mono font-medium text-primary hover:underline">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-text">{{ $installment->created_at?->translatedFormat('d M Y · H.i') }} WIB</dd>
                    </div>
