<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-start gap-3">
            <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-secondary/10 text-secondary">
                <x-ui.icon name="credit-card" class="h-6 w-6" />
            </span>
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-text">Angsuran</h2>
                <p class="mt-0.5 text-sm text-muted">Riwayat pembayaran angsuran pinjaman. Koreksi hanya lewat reversal.</p>
            </div>
        </div>
    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="relative w-full lg:max-w-xs">
            ...
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="Cari no. angsuran, pinjaman, anggota…"
        ...
        <div class="overflow-x-auto rounded-2xl">
            <table class="w-full text-sm">
                <thead class="bg-bg text-xs font-medium uppercase tracking-wide text-muted">
                    <tr>
                        <th class="px-5 py-3 text-left">No. Angsuran</th>
                ...
                <tbody wire:loading.remove class="divide-y divide-border">
                    @forelse ($installments as $installment)
                        <tr class="transition hover:bg-bg/60" wire:key="ins-{{ $installment->id }}">
                            ...
                            <td class="px-5 py-4">
                                <span class="text-text">{{ $installment->loan?->member?->full_name ?? '—' }}</span>
                                <p class="font-mono text-xs text-muted">{{ $installment->loan?->member?->member_number }}</p>
                            </td>
                            <td class="px-5 py-4 font-mono text-xs text-muted">{{ $installment->loan?->loan_number ?? '—' }}</td>
                            <td class="px-5 py-4 text-right font-medium tabular-nums {{ $installment->is_reversal ? 'text-danger' : 'text-success' }}">
                                {{ $installment->is_reversal ? '−' : '+' }}Rp {{ number_format((float) $installment->amount_paid, 0, ',', '.') }}
                            </td>
                            <td class="px-5 py-4 text-text">
                                {{ $installment->payment_date?->translatedFormat('d M Y') }}
                                <span class="block text-xs text-muted">{{ $installment->created_at?->translatedFormat('H.i') }} WIB</span>
                            </td>
                            <td class="px-5 py-4 text-muted">{{ $paymentMethods[$installment->payment_method] ?? $installment->payment_method }}</td>
                            <td class="px-5 py-4">
                                <div class="flex justify-end">
                                    <x-ui.dropdown>
                                        <x-ui.dropdown-item icon="eye" :href="route('installments.show', $installment)" wire:navigate>Lihat Detail</x-ui.dropdown-item>
                                        <x-ui.dropdown-item icon="printer" :href="route('installments.receipt', $installment)">Cetak Kuitansi</x-ui.dropdown-item>

                                        @if ($this->canReverse($installment))
                                            <div class="my-1 border-t border-border"></div>
                                            <x-ui.dropdown-item icon="arrow-uturn-left" variant="danger"
                                                wire:click="openReverse('{{ $installment->id }}')">Reversal</x-ui.dropdown-item>
                                        @endif
                                    </x-ui.dropdown>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-5 py-16">
                                <div class="flex flex-col items-center justify-center text-center">
                                    <div class="grid h-14 w-14 place-items-center rounded-2xl bg-secondary/10 text-secondary">
                                        <x-ui.icon name="credit-card" class="h-7 w-7" />
                                    </div>
                                    <h4 class="mt-4 text-sm font-semibold text-text">
                                        {{ $this->hasActiveFilters() ? 'Tidak ada angsuran yang cocok' : 'Belum ada pembayaran angsuran' }}
                                    </h4>
                                    <p class="mt-1 max-w-xs text-xs text-muted">
                                        {{ $this->hasActiveFilters()
                                            ? 'Coba ubah kata kunci atau filter.'
                                            : 'Catat pembayaran angsuran pertama.' }}
                                    </p>
                                    @can('create_installment')
                                        @unless ($this->hasActiveFilters())
                                            <x-ui.button :href="route('installments.create')" wire:navigate class="mt-4 h-9 px-3">
                                                <x-ui.icon name="plus" class="h-4 w-4" /> Bayar Angsuran
                                            </x-ui.button>
                                        @endunless
                                    @endcan
                                </div>
        ...
        <div x-show="show"
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted">Jadwal kembali Belum Bayar. Jika ini pelunasan, pengembalian SWP &amp; Tab. Berjangka ikut dibatalkan.</p>
                </div>
            </div>
