            ...
            <x-ui.card>
                ...
                <div class="grid gap-3 sm:grid-cols-3">
                    <div class="rounded-xl bg-primary/5 px-4 py-4 ring-1 ring-inset ring-primary/15 sm:col-span-1">
                        ...
                        <p class="mt-1 text-2xl font-bold tabular-nums text-primary">Rp {{ number_format((float) $loan->disbursed_amount, 0, ',', '.') }}</p>
                    </div>
                    <div class="rounded-xl bg-bg/50 px-4 py-4 ring-1 ring-inset ring-border">
                        ...
                        <p class="mt-1 text-xl font-bold tabular-nums text-text">Rp {{ number_format((float) $loan->principal_amount, 0, ',', '.') }}</p>
                    </div>
                    <div class="rounded-xl bg-bg/50 px-4 py-4 ring-1 ring-inset ring-border">
                        <p class="text-xs font-medium uppercase tracking-wide text-muted">Sisa Pokok</p>
                        <p class="mt-1 text-xl font-bold tabular-nums text-text">Rp {{ number_format((float) $progress['remaining'], 0, ',', '.') }}</p>
                    </div>
                </div>
                ...
                <dl class="mt-5 grid grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-text">{{ $loan->disbursement_date?->translatedFormat('d M Y') }}</dd>
                    </div>
                    <div>
                        <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-text">Rp {{ number_format((float) $loan->admin_fee, 0, ',', '.') }}</dd>
                    </div>
                    <div>
                        <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
                            <x-ui.icon name="wallet" class="h-3.5 w-3.5" /> SWP
                        </dt>
                        <dd class="mt-1 text-sm text-text">Rp {{ number_format((float) $loan->swp_amount, 0, ',', '.') }}</dd>
                    </div>
                    <div>
                        <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-text">{{ $loan->created_at?->translatedFormat('d M Y · H.i') }} WIB</dd>
                    </div>
                    ...
                    <div class="col-span-2 rounded-xl border border-dashed border-border px-4 py-3">
                        ...
                        <dd class="mt-2 flex flex-wrap gap-x-6 gap-y-1.5 text-sm">
                            <span class="text-muted">Pokok <span class="font-medium tabular-nums text-text">Rp {{ number_format((float) $loan->monthly_principal, 0, ',', '.') }}</span></span>
                            <span class="text-muted">Jasa <span class="font-medium tabular-nums text-text">Rp {{ number_format((float) $loan->monthly_interest, 0, ',', '.') }}</span></span>
                            <span class="text-muted">Tab. Berjangka <span class="font-medium tabular-nums text-text">Rp {{ number_format((float) $loan->monthly_time_deposit, 0, ',', '.') }}</span></span>
                        </dd>
                    </div>
                ...
                <div class="border-b border-border p-5">
                    ...
                    <div class="mt-4 space-y-1.5">
                        ...
                        <div class="h-2.5 w-full overflow-hidden rounded-full bg-border/60">
                            <div class="h-2.5 rounded-full bg-linear-to-r from-primary to-secondary transition-all" style="width: {{ $progress['percent'] }}%"></div>
                        </div>
                        <p class="text-xs text-muted">Sisa pokok Rp {{ number_format((float) $progress['remaining'], 0, ',', '.') }}</p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-border">
                            @forelse ($schedules as $schedule)
                                ...
                                @php($payment = $schedule->status === 'Terbayar' ? $this->actualPayment($schedule) : null)
                                @php($clickable = $payment && $canViewInstallment)
                                <tr wire:key="sched-{{ $schedule->id }}"
                                    @class(['transition hover:bg-bg/50', 'cursor-pointer' => $clickable])
                                    @if ($clickable)
                                        @click="window.Livewire.navigate('{{ route('installments.show', $payment) }}')"
                                        title="Lihat detail pembayaran {{ $payment->installment_number }}"
                                    @endif>
                                    <td class="px-5 py-3 font-medium text-text">{{ $schedule->installment_seq }}</td>
                                    <td class="px-5 py-3 text-text">{{ $schedule->due_date?->translatedFormat('d M Y') }}</td>
                                    <td class="px-5 py-3 text-right tabular-nums text-text">Rp {{ number_format((float) $schedule->total_due, 0, ',', '.') }}</td>
                                    <td class="px-5 py-3 text-right tabular-nums {{ $payment ? 'font-medium text-success' : 'text-muted' }}">
                                    ...
                                    <td class="px-5 py-3 text-muted">{{ $payment?->payment_date?->translatedFormat('d M Y') ?? '—' }}</td>
                                ...
                                <tr>
                                    <td colspan="7" class="px-5 py-10">
                                        <div class="flex flex-col items-center justify-center text-center">
                                            ...
                                            <p class="mt-1 text-xs text-muted">Klik <span class="font-medium text-text">Tampilkan rancangan angsuran</span> untuk melihat jadwal lengkap.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
