...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="mt-0.5 text-sm text-muted">Daftar pinjaman anggota. Pinjaman immutable — koreksi salah-input
                    lewat reversal.</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div
            class="relative overflow-hidden rounded-2xl border border-border bg-linear-to-br from-primary/10 via-surface to-surface p-5 shadow-sm">
            <div class="absolute -right-4 -top-6 h-20 w-20 rounded-full bg-primary/10 blur-2xl" aria-hidden="true">
            </div>
            ...
            <p class="relative mt-2 text-3xl font-bold tabular-nums text-text">
        ...
        <div class="rounded-2xl border border-border bg-surface p-5 shadow-sm">
            ...
            <p class="mt-2 text-3xl font-bold tabular-nums text-text">
                {{ number_format($stats['settled'], 0, ',', '.') }}</p>
        </div>
        ...
        <div
            class="relative overflow-hidden rounded-2xl border border-border bg-linear-to-br from-secondary/10 via-surface to-surface p-5 shadow-sm">
            <div class="absolute -right-4 -top-6 h-20 w-20 rounded-full bg-secondary/10 blur-2xl" aria-hidden="true">
            </div>
            ...
            <p class="relative mt-2 text-2xl font-bold tabular-nums text-text">Rp
                {{ number_format((float) $stats['outstanding'], 0, ',', '.') }}</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody wire:loading.delay class="divide-y divide-border">
                    @for ($i = 0; $i < 5; $i++)
                        <tr>
                            <td class="px-5 py-4">
                                <div class="h-4 w-28 animate-pulse rounded bg-border/60"></div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="h-4 w-36 animate-pulse rounded bg-border/60"></div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="h-4 w-24 animate-pulse rounded bg-border/60"></div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="ml-auto h-4 w-20 animate-pulse rounded bg-border/60"></div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="h-4 w-16 animate-pulse rounded bg-border/60"></div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="h-4 w-24 animate-pulse rounded bg-border/60"></div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="ml-auto h-8 w-8 animate-pulse rounded-lg bg-border/60"></div>
                            </td>
                        </tr>
                    @endfor
                </tbody>
                        ...
                        <tr class="transition hover:bg-bg/60" wire:key="loan-{{ $loan->id }}">
                            <td class="px-5 py-4">
                                <a href="{{ route('loans.show', $loan) }}" wire:navigate
                                    class="font-mono text-xs font-medium text-text hover:text-primary">
                                    {{ $loan->loan_number }}
                                </a>
                                @if ($loan->overdue_count > 0)
                                    <div class="mt-1"><x-ui.badge color="danger">{{ $loan->overdue_count }}
                                            terlewat</x-ui.badge></div>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                <x-ui.badge
                                    :color="$typeColor($loan->loan_type)">{{ $loanTypes[$loan->loan_type] ?? $loan->loan_type }}</x-ui.badge>
                            </td>
                            <td class="px-5 py-4 text-right font-medium tabular-nums text-text">
                                Rp {{ number_format((float) $loan->principal_amount, 0, ',', '.') }}
                            </td>
                            <td class="px-5 py-4">
                                @if ($loan->status === \App\Enums\LoanStatus::Lunas)
                                    <x-ui.badge color="success"><x-ui.icon name="check" class="h-3 w-3" />
                                        Lunas</x-ui.badge>
                                @elseif ($loan->status === \App\Enums\LoanStatus::Dibatalkan)
                                    <x-ui.badge color="neutral"><x-ui.icon name="x" class="h-3 w-3" />
                                        Dibatalkan</x-ui.badge>
                                @else
                                    ...
                                    <div class="w-32 space-y-1">
                                        <div class="flex items-center justify-between text-xs">
                                            ...
                                            <span
                                                class="tabular-nums text-muted">{{ $loan->schedules_paid }}/{{ $loan->schedules_total }}
                                                angsuran</span>
                                        </div>
                                        <div class="h-1.5 w-full overflow-hidden rounded-full bg-border/60">
                                            <div class="h-1.5 rounded-full {{ $loan->overdue_count > 0 ? 'bg-danger' : 'bg-linear-to-r from-primary to-secondary' }}"
                                                style="width: {{ $pct }}%"></div>
                                        </div>
                                    </div>
                                @endif
                            ...
                            <td class="px-5 py-4">
                                <div class="flex justify-end">
                                    <x-ui.dropdown>
                                        <x-ui.dropdown-item icon="eye" :href="route('loans.show', $loan)" wire:navigate>Lihat
                                            Detail</x-ui.dropdown-item>
                                        <x-ui.dropdown-item icon="printer" :href="route('loans.receipt', $loan)">Tanda
                                            Terima</x-ui.dropdown-item>

                                        @if ($this->canCorrect($loan))
                                            <div class="my-1 border-t border-border"></div>
                                            <x-ui.dropdown-item icon="arrow-uturn-left" variant="danger"
                                                wire:click="openCorrect('{{ $loan->id }}')">Koreksi
                                                Salah-Input</x-ui.dropdown-item>
                                        @endif
                                    </x-ui.dropdown>
                                </div>
                            </td>
        ...
        <div x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted">Hanya untuk pinjaman salah input yang belum punya angsuran.
                        Record &amp; jadwalnya dihapus, dicatat di audit.</p>
                </div>
            </div>
