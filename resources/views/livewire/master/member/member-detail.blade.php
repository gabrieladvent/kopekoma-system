...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex items-start gap-3">
            <span
                class="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-secondary/15 text-base font-semibold text-secondary">
            ...
            <div>
                ...
                <p class="mt-0.5 text-sm text-muted">
                    {{ $member->position ? $member->position . ' · ' : '' }}{{ $member->agency?->agency_name }}</p>
            </div>
        </div>
    ...
    <x-ui.card>
        ...
        <div class="rounded-xl bg-bg px-4 py-3">
            ...
            <p class="mt-1 text-2xl font-bold tabular-nums text-text">Rp
                {{ number_format((int) $member->mandatory_savings_amount, 0, ',', '.') }}</p>
        </div>
        ...
        <div class="mt-6 grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div>
                ...
                <dl class="mt-3 space-y-3 text-sm">
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">NIK</dt>
                        <dd class="font-mono text-text">{{ $member->nik }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">NIP</dt>
                        <dd class="text-text">{{ $member->nip ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Tempat, Tgl Lahir</dt>
                        <dd class="text-right text-text">{{ $member->birth_place }},
                            {{ $member->birth_date?->translatedFormat('d M Y') }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Jenis Kelamin</dt>
                        <dd class="text-text">{{ $member->gender === 'L' ? 'Laki-laki' : 'Perempuan' }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div>
                ...
                <dl class="mt-3 space-y-3 text-sm">
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">OPD / Instansi</dt>
                        <dd class="text-right text-text">{{ $member->agency?->agency_name ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Golongan</dt>
                        <dd class="text-right text-text">
                            {{ $member->grade ? $member->grade->code . ' — ' . $member->grade->name : '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Jabatan</dt>
                        <dd class="text-text">{{ $member->position ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Status Kepegawaian</dt>
                        <dd><x-ui.badge color="neutral">{{ $member->employment_status }}</x-ui.badge></dd>
                    </div>
                </dl>
            </div>
            ...
            <div>
                ...
                <dl class="mt-3 space-y-3 text-sm">
                    <div class="flex justify-between gap-4">
                        ...
                        <dd class="text-text">{{ $member->phone_number ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">No. Rekening Gaji</dt>
                        <dd class="font-mono text-text">{{ $member->payroll_account_number }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Bank</dt>
                        <dd class="text-text">{{ $member->bank_name ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Alamat</dt>
                        <dd class="max-w-[60%] text-right text-text">{{ $member->address }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div>
                ...
                <dl class="mt-3 space-y-3 text-sm">
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Ahli Waris</dt>
                        <dd class="text-right text-text">{{ $member->heir_name }} <span
                                class="text-muted">({{ $member->heir_relationship }})</span></dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">No. HP Ahli Waris</dt>
                        <dd class="text-text">{{ $member->heir_phone_number ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Tgl Bergabung</dt>
                        <dd class="text-text">{{ $member->join_date?->translatedFormat('d M Y') }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-muted">Tgl Keluar</dt>
                        <dd class="text-text">{{ $member->exit_date?->translatedFormat('d M Y') ?: '—' }}</dd>
                    </div>
                </dl>
            </div>
        </div>
            ...
            <div class="mt-2 divide-y divide-border">
                @forelse ($documents as $doc)
                    <div class="flex items-center gap-3 py-3" wire:key="doc-{{ $doc->id }}">
                        ...
                        <a href="{{ route('media.show', $doc) }}" target="_blank" rel="noopener" title="Lihat"
