...
<div class="space-y-6" x-data
    ...
    <div class="flex items-start gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-text">{{ $isEdit ? 'Edit Anggota' : 'Tambah Anggota' }}
            </h2>
            <p class="mt-0.5 text-sm text-muted">No. Anggota <span
                    class="font-mono font-medium text-text">{{ $member_number }}</span> — digenerate otomatis, tidak
                dapat diubah.</p>
        </div>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <x-ui.card>
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    <x-ui.input label="Nama Lengkap" name="full_name" wire:model="full_name"
                        placeholder="Nama sesuai KTP" :error="$errors->first('full_name')" />
                </div>

                <div class="space-y-1">
                    <label for="nik" class="block text-sm font-medium text-text">NIK</label>
                    <input id="nik" type="text" inputmode="numeric" wire:model="nik" placeholder="16 digit"
                        maxlength="16" @class([
                    ...
                    @error('nik')
                    <p class="text-xs text-danger">{{ $message }}</p>@else<p class="text-xs text-muted">Nomor Induk
                            Kependudukan, 16 digit, unik.</p>
                    @enderror
                </div>

                <x-ui.input label="NIP" name="nip" wire:model="nip" placeholder="Wajib untuk ASN"
                    :error="$errors->first('nip')" :hint="$errors->has('nip') ? null : 'Wajib untuk ASN, opsional untuk Honorer.'" />

                <x-ui.input label="Tempat Lahir" name="birth_place" wire:model="birth_place"
                    placeholder="Kota kelahiran" :error="$errors->first('birth_place')" />

                <x-ui.input label="Tanggal Lahir" name="birth_date" type="date" wire:model="birth_date"
                    max="{{ now()->toDateString() }}" :error="$errors->first('birth_date')" />
        ...
        <x-ui.card>
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="space-y-1">
                    ...
                    <select id="agency_id" wire:model="agency_id" @class([
                ...
                <div class="space-y-1">
                    ...
                    <select id="grade_id" wire:model.live="grade_id" @class([
                        'h-10 w-full rounded-lg border bg-surface px-3 text-sm text-text transition focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none',
                    ...
                    @error('grade_id')
                    <p class="text-xs text-danger">{{ $message }}</p>@else<p class="text-xs text-muted">Menentukan
                            default nominal simpanan wajib (snapshot).</p>
                    @enderror
                </div>

                <div class="space-y-1">
                    <label for="employment_status" class="block text-sm font-medium text-text">Status
                        Kepegawaian</label>
                ...
                <x-ui.input label="Jabatan" name="position" wire:model="position" placeholder="Opsional"
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="space-y-1" x-data="{
                    ...
                    <div @class([
                        ...
                        'border-danger focus-within:ring-danger' => $errors->has(
                            'mandatory_savings_amount'),
                        'opacity-60' => !$canOverride,
                    ])>
                        <span class="pl-3 text-sm text-muted">Rp</span>
                        <input id="msa" type="text" inputmode="numeric" :value="display"
                            @input="onInput($event)" placeholder="50.000" @disabled(!$canOverride)
                    ...
                    @error('mandatory_savings_amount')
                        <p class="text-xs text-danger">{{ $message }}</p>
                    @else<p class="text-xs text-muted">
                            {{ $canOverride ? 'Default dari golongan; boleh di-override.' : 'Default dari golongan. Hanya Pengurus ke atas yang dapat mengubah.' }}
                        </p>
                    @enderror
                </div>
                ...
                <x-ui.input label="No. Rekening Gaji" name="payroll_account_number"
                    wire:model="payroll_account_number" placeholder="Rekening tujuan potong gaji" :error="$errors->first('payroll_account_number')" />

                <x-ui.input label="Nama Bank" name="bank_name" wire:model="bank_name" placeholder="Opsional"
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="space-y-1">
                    ...
                    <div class="flex items-center rounded-lg border bg-surface transition focus-within:ring-2 focus-within:ring-primary"
                        @class([
                            'border-border' => !$errors->has('phone_number'),
                            'border-danger focus-within:ring-danger' => $errors->has('phone_number'),
                        ])>
                        <span class="pl-3 text-sm text-muted">+62</span>
                        <input id="phone_number" type="tel" inputmode="numeric" wire:model="phone_number"
                            placeholder="81234567890"
                            class="h-10 w-full rounded-lg bg-transparent px-2 text-sm text-text placeholder:text-muted focus-visible:outline-none">
                    </div>
                    @error('phone_number')
                    <p class="text-xs text-danger">{{ $message }}</p>@else<p class="text-xs text-muted">Tanpa
                            angka 0 di depan. Disimpan dengan awalan +62.</p>
                    @enderror
                </div>
        ...
        <x-ui.card>
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                <x-ui.input label="Tanggal Bergabung" name="join_date" type="date" wire:model="join_date"
                ...
                @if (in_array($statusForm, ['Keluar', 'Meninggal'], true))
                    <x-ui.input label="Tanggal Keluar" name="exit_date" type="date" wire:model="exit_date"
                        :error="$errors->first('exit_date')" :hint="$errors->has('exit_date') ? null : 'Wajib bila status Keluar / Meninggal.'" />
                @else
                    <div class="hidden sm:block"></div>
                @endif
            </div>
        ...
        <x-ui.card>
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                <x-ui.input label="Nama Ahli Waris" name="heir_name" wire:model="heir_name"
                    placeholder="Nama lengkap" :error="$errors->first('heir_name')" />

                <div class="space-y-1">
                    <label for="heir_relationship" class="block text-sm font-medium text-text">Hubungan</label>
                    <select id="heir_relationship" wire:model="heir_relationship" @class([
                        'h-10 w-full rounded-lg border bg-surface px-3 text-sm text-text transition focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none',
                        'border-border' => !$errors->has('heir_relationship'),
                        'border-danger focus-visible:ring-danger' => $errors->has(
                            'heir_relationship'),
                    ])>
                    ...
                    @error('heir_relationship')
                        <p class="text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>

                <div class="space-y-1">
                    <label for="heir_phone_number" class="block text-sm font-medium text-text">No. HP Ahli
                        Waris</label>
                    <div class="flex items-center rounded-lg border bg-surface transition focus-within:ring-2 focus-within:ring-primary"
                        @class([
                            'border-border' => !$errors->has('heir_phone_number'),
                            'border-danger focus-within:ring-danger' => $errors->has(
                                'heir_phone_number'),
                        ])>
                        <span class="pl-3 text-sm text-muted">+62</span>
                        <input id="heir_phone_number" type="tel" inputmode="numeric"
                            wire:model="heir_phone_number" placeholder="81234567890"
                            class="h-10 w-full rounded-lg bg-transparent px-2 text-sm text-text placeholder:text-muted focus-visible:outline-none">
                    </div>
                    @error('heir_phone_number')
                        <p class="text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
            </div>
        ...
        <x-ui.card>
            <div class="flex items-center gap-2 border-b border-border pb-3">
                ...
                <h3 class="text-sm font-semibold text-text">Dokumen <span
                        class="font-normal text-muted">(opsional)</span></h3>
            </div>
            ...
            <div class="mt-4 space-y-4">
                        ...
                        @foreach ($documents as $doc)
                            <div class="flex items-center gap-3 px-3 py-2.5" wire:key="exist-{{ $doc->id }}">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs text-muted">{{ $doc->human_readable_size }} ·
                                        {{ $doc->created_at?->translatedFormat('d M Y H:i') }}</p>
                                </div>
                                <a href="{{ route('media.show', $doc) }}" target="_blank" rel="noopener"
                                    title="Lihat"
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-xs text-muted">KTP, SK, formulir, dll. PDF/JPG/PNG, maks 5 MB per berkas. Bisa pilih
                        beberapa berkas sekaligus atau menambah bertahap. Dilampirkan saat form disimpan.</p>
                    @error('newUploads.*')
                        <p class="text-xs text-danger">{{ $message }}</p>
                    @enderror
                    @error('uploads.*')
                        ...
                        <div class="divide-y divide-border rounded-xl border border-dashed border-border">
                            @foreach ($uploads as $i => $file)
                                <div class="flex items-center gap-3 px-3 py-2"
                                    ...
                                    <span
                                        class="min-w-0 flex-1 truncate text-sm text-text">{{ $file->getClientOriginalName() }}</span>
