@php
    ...
    $statusBadge = match (\App\Filament\Resources\SavingsWithdrawalResource::statusColor($withdrawal->status)) {
        'info' => 'primary',
        'success' => 'success',
        'danger' => 'danger',
        default => 'neutral',
    ...
    $steps = [
        [
            'label' => 'Diajukan', 'desc' => 'Pengajuan dibuat sebagai draft.', 'icon' => 'document',
            'state' => 'done', 'time' => $withdrawal->created_at, 'who' => $withdrawal->recordedBy?->name,
        ],
        [
            'label' => 'Disetujui (ACC)', 'desc' => 'Mata kedua sebelum dana keluar.', 'icon' => 'check',
            'state' => $accDone ? 'done' : (($status === 'draft' && ! $rejected) ? 'current' : 'upcoming'),
            'time' => $withdrawal->approved_at, 'who' => $withdrawal->approvedBy?->name,
        ],
        [
            'label' => 'Dana Cair', 'desc' => 'Saldo anggota berkurang.', 'icon' => 'banknotes',
            'state' => $cairDone ? 'done' : (($status === 'acc' && ! $rejected) ? 'current' : 'upcoming'),
            'time' => $withdrawal->disbursed_at, 'who' => null,
        ],
    ];

    $node = [
        'done'     => ['ring' => 'bg-success text-white', 'line' => 'bg-success', 'label' => 'text-text'],
        'current'  => ['ring' => 'border-2 border-primary bg-primary/10 text-primary', 'line' => 'bg-border', 'label' => 'text-text'],
        'upcoming' => ['ring' => 'border-2 border-border bg-surface text-muted', 'line' => 'bg-border', 'label' => 'text-muted'],
    ];
@endphp
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <h2 class="mt-2 text-2xl font-bold tracking-tight text-text">{{ $withdrawal->member?->full_name ?? '—' }}</h2>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
            <x-ui.card>
                <div class="rounded-xl px-4 py-4 {{ $withdrawal->is_reversal ? 'bg-success/5 ring-1 ring-inset ring-success/15' : 'bg-primary/5 ring-1 ring-inset ring-primary/15' }}">
                    <p class="text-xs font-medium uppercase tracking-wide {{ $withdrawal->is_reversal ? 'text-success' : 'text-primary' }}">
                ...
                <dl class="mt-5 grid grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-text">{{ $withdrawal->created_at?->translatedFormat('d M Y · H.i') }} WIB</dd>
                    </div>
                    <div>
                        <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
                    ...
                    @if ($withdrawal->is_reversal && $withdrawal->reversalOf)
                        <div class="col-span-2 rounded-lg border border-dashed border-border px-3 py-2.5">
                            ...
                            <dd class="mt-1">
                                <a href="{{ route('savings.withdrawals.show', $withdrawal->reversalOf) }}" wire:navigate
        ...
        <div class="space-y-6">
            ...
            <x-ui.card>
                ...
                @if ($rejected)
                    <div class="mt-4 flex items-start gap-2.5 rounded-xl bg-danger/5 px-3 py-2.5 text-xs text-danger ring-1 ring-inset ring-danger/15">
                        <x-ui.icon name="x" class="mt-0.5 h-4 w-4 shrink-0" />
                        <span>Pengajuan ini <span class="font-semibold">ditolak</span> dan bersifat final. Saldo tidak berubah.</span>
                    </div>
                @endif
                ...
                <ol class="mt-4 space-y-0">
                    @foreach ($steps as $i => $step)
                        ...
                        <li class="relative flex gap-3 pb-6 last:pb-0">
                            {{-- Garis penghubung --}}
                            @unless ($loop->last)
                                <span class="absolute left-3.75 top-8 h-[calc(100%-1.5rem)] w-0.5 {{ $n['line'] }}"></span>
                            @endunless
                            {{-- Node --}}
                            <span class="relative z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full {{ $n['ring'] }} {{ $step['state'] === 'current' ? 'ring-4 ring-primary/15' : '' }}">
                                @if ($step['state'] === 'done')
                            ...
                            <div class="min-w-0 flex-1 pt-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($step['state'] === 'current')
                                        <span class="inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-primary">Menunggu</span>
                                    @endif
                                </div>
                                <p class="mt-0.5 text-xs text-muted">{{ $step['desc'] }}</p>
                                @if ($step['time'])
                                    <p class="mt-1 text-[11px] text-muted">
                                        {{ $step['time']->translatedFormat('d M Y · H.i') }} WIB{{ $step['who'] ? ' · '.$step['who'] : '' }}
                                    </p>
                                @endif
                            </div>
                        </li>
